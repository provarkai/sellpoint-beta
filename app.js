...
$("productForm").onsubmit=async e=>{e.preventDefault();try{const file=$("pImage")?.files?.[0];const image=file?await readFileAsDataUrl(file):undefined;const payload={name:$("pName").value.trim(),price:+$("pPrice").value,stock:+$("pStock").value,category:$("pCat").value.trim(),type:$("pType")?.value||"Product",deliveryLink:$("pDelivery")?.value.trim()||"",deliveryNote:$("pNote")?.value.trim()||""};if(image!==undefined)payload.image=image;
...
if($("storefrontForm"))$("storefrontForm").onsubmit=async e=>{e.preventDefault();try{const updated=await api("PUT","/api/business/storefront",{enabled:$("storefrontEnabled").checked,slug:$("storefrontSlug").value.trim()});Object.assign(state,updated);render();toast("Storefront settings saved")}catch(err){toast(err.message)}};
