...
function show(tab){document.querySelectorAll(".tab").forEach(b=>b.classList.toggle("active",b.dataset.tab===tab));document.querySelectorAll(".page").forEach(p=>p.classList.toggle("active",p.id===tab));$("title").textContent={dash:"Dashboard",products:"Products",customers:"Customers",orders:"Orders",invoice:"Receipt",ai:"AI Tools",reports:"Reports",logistics:"Logistics",settings:"Settings"}[tab];if(tab==="reports")loadReports();if(tab==="ai")refreshAiUsage();if(tab==="logistics")renderLogistics()}
...
function editProduct(id){const p=product(id);if(!p)return;editingProductId=id;$("pName").value=p.name;$("pPrice").value=p.price;$("pStock").value=p.stock;$("pCat").value=p.category||"";$("pType").value=p.type||"Product";updateProductFields();$("pDelivery").value=p.deliveryLink||"";$("pNote").value=p.deliveryNote||"";if($("pDescription"))$("pDescription").value=p.description||"";$("pFormTitle").textContent="Edit product / service";$("pSubmitBtn").textContent="Update Item";$("pCancelEdit").style.display="inline-grid";show("products")}
function cancelEditProduct(){editingProductId=null;$("productForm").reset();updateProductFields();$("pFormTitle").textContent="Add product / service";$("pSubmitBtn").textContent="Save Item";$("pCancelEdit").style.display="none"}
if($("pCancelEdit"))$("pCancelEdit").onclick=cancelEditProduct;
$("productForm").onsubmit=async e=>{e.preventDefault();try{const file=$("pImage")?.files?.[0];const image=file?await readFileAsDataUrl(file):undefined;const payload={name:$("pName").value.trim(),price:+$("pPrice").value,stock:+$("pStock").value,category:$("pCat").value.trim(),type:$("pType")?.value||"Product",deliveryLink:$("pDelivery")?.value.trim()||"",deliveryNote:$("pNote")?.value.trim()||"",description:$("pDescription")?.value.trim()||""};if(image!==undefined)payload.image=image;
  ...
  else{const created=await api("POST","/api/products",{...payload,image:image||""});state.products.unshift(created);e.target.reset();updateProductFields();render();toast("Item saved")}
...
if($("cImportCsv"))$("cImportCsv").onchange=async e=>{const file=e.target.files?.[0];if(!file)return;try{await importCsv(file,"/api/customers",row=>({name:(row.name||"").trim(),phone:(row.phone||"").replace(/\D/g,""),location:(row.location||"").trim()}),"customers")}catch(err){toast(err.message)}e.target.value=""};
