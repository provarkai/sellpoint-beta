...
const TAB_PERMISSIONS={products:"products.write",customers:"customers.write",orders:"orders.write",pos:"pos.use",expenses:"expenses.write",suppliers:"suppliers.write",settings:"settings.write"};
...
const PERMISSION_DEFS=[["products.write","Products"],["customers.write","Customers"],["orders.write","Orders"],["pos.use","POS"],["invoices.use","Invoices"],["expenses.write","Expenses"],["reports.read","Reports"],["suppliers.write","Suppliers"],["campaigns.send","Campaigns"],["coupons.manage","Coupons"],["logistics.manage","Logistics"],["settings.write","Settings"]];
const PERMISSION_PRESETS={manager:["products.write","customers.write","orders.write","pos.use","invoices.use","expenses.write","reports.read","suppliers.write","campaigns.send","coupons.manage","logistics.manage","settings.write"],sales_staff:["customers.write","orders.write","pos.use","invoices.use"],accountant:["reports.read","expenses.write","invoices.use"]};
...
function renderActivationChecklist(){
  ...
  const items=[["Add payment details",!!state.paymentDetails,"settings"],["Add your first product",state.products.length>0,"products"],["Add your first customer",state.customers.length>0,"customers"],["Create your first order",state.orders.length>0,"orders"],["Use your store AI",localStorage.getItem("sp_used_store_ai")==="1","ai"]];
...
(async()=>{const ctx=await window.Auth.requireSession();if(!ctx)return;authToken=ctx.session.access_token;userEmail=ctx.session.user.email;loadState().then(render).then(loadInsight).then(loadMonthlyPL)})().catch(err=>toast(err.message||"Something went wrong loading this page."));
