...
$("productForm").onsubmit=async e=>{e.preventDefault();try{const file=$("pImage")?.files?.[0];const image=file?await readFileAsDataUrl(file):"";const created=await api("POST","/api/products",{name:$("pName").value.trim(),price:+$("pPrice").value,stock:+$("pStock").value,category:$("pCat").value.trim(),type:$("pType")?.value||"Product",deliveryLink:$("pDelivery")?.value.trim()||"",deliveryNote:$("pNote")?.value.trim()||"",image});state.products.unshift(created);e.target.reset();updateProductFields();render();toast("Item saved")}catch(err){toast(err.message)}};
