// Paystack provider - implements the shared interface documented in
// server/payments/index.js. Behavior here is unchanged from the original
// single-provider server/payments.js; only the file location and the
// getKeys() indirection (so keys can come from platform_settings, not just
// env vars) are new.
...
async function initializeTransaction({ email, plan, billingCycle, amountNaira, reference, callbackUrl, businessId, config }) {
  ...
  return { authorizationUrl: body.data.authorization_url, amount: amountNaira, reference };
}
...
async function initializeAddonTransaction({ email, addonType, reference, callbackUrl, businessId, config }) {
  ...
  const body = await res.json();
  if (!res.ok || !body.status) throw new Error(body.message || "Paystack initialize failed");
  return { authorizationUrl: body.data.authorization_url, amount, reference };
}
...
// public URL for Paystack to reach with a webhook. Returns a shape normalized
// across every provider - see index.js's NORMALIZED_SHAPE comment.
async function verifyTransaction(reference, config) {
  ...
  const d = body.data;
  return {
    status: d.status === "success" ? "success" : d.status,
    amount: d.amount, // kobo, same unit finalizeIfSuccessful already expects
...
async function initializeStorefrontCheckout({ email, amountNaira, absorbFees, subaccountCode, reference, callbackUrl, businessId, orderId, plan, config }) {
  ...
  const body = await res.json();
  if (!res.ok || !body.status) throw new Error(body.message || "Paystack initialize failed");
  return { authorizationUrl: body.data.authorization_url, amount: chargeAmount, platformCut, reference };
}

// Normalizes a raw Paystack webhook payload into the same shape
...
function extractWebhookEvent(body) {
  ...
  const d = body.data || {};
  return { status: "success", amount: d.amount, currency: d.currency || "NGN", reference: d.reference, metadata: d.metadata || {} };
}
