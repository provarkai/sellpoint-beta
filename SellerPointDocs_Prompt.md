# SellersPoint Docs — Product & Automation Brief (v15)
...
**Onboarding vs. Manage — sidebar grouping added in v14, enforced as a real gate in v15.** Business Registration and Verification are the one-time-per-business setup work — get registered, get verified — so the sidebar labels them **Onboarding**. Tax Tools, Trackers, and Document Vault are what a seller comes back to repeatedly after that's done, labeled **Manage**. Overview and the Compliance Calendar sit above both groups, ungrouped, since they're dashboard-level surfaces a seller might open regardless of which phase they're in.

**v15: Business Registration is a one-time process that ends at SCUML, and nothing in Manage makes sense before it's done — so it's now an actual gate, not just a label.** `isOnboardingComplete()` is true only once `regStatus === "approved"` **and** `scuml.status === "approved"` — SCUML is explicitly the last step, matching how it works in reality (issued once by the EFCC, after the underlying Business Name/LLC/Partnership registration is already done). Until both are true:
- Tax Tools, Trackers, and Document Vault show a 🔒 next to their nav label and are visibly dimmed.
...
- The Overview "Onboarding" card (renamed from "Your Business Setup") and the SCUML congrats banner are now *computed* from the same three fields (`regStatus`, `tinIssued`, `scuml.status`) that drive the gate, not separately hardcoded — so the progress bar, step checklist, congrats message, and nav locks can never drift out of sync with each other or with what's actually navigable. Before this pass they were independent hardcoded fields on the business record; fixing that was necessary once real navigation started depending on the same underlying facts.
- Verification and Compliance Calendar stay unlocked throughout — Verification is itself one of the two onboarding steps (needed to progress, not gated by completion), and the Calendar's statutory-deadline preview is useful to see before registration finishes, not just after. Templates also stay unlocked (reachable via the Overview quick action and Vault prompt) since generic legal templates don't depend on a TIN or SCUML certificate existing.
