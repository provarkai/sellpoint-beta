# SellersPoint Docs — Product & Automation Brief (v16)
...
## 2. Full Service Catalog — three pillars (restructured in v16)

The catalog is organized around three pillars now, matching how the partner pricing packages actually work rather than an internal Corporate-Legal/Tax-Regulatory split that didn't map cleanly to what a seller experiences:

| Pillar | Services | Fulfillment |
|---|---|---|
| **Business Registration** *(one-time)* | CAC Business Name, full LLC incorporation, or Partnership registration — exactly one per business, never more than one at a time. **TIN is generated and included automatically as part of this package** — it isn't billed, fulfilled, or verified as a separate step. **SCUML registration** is the step that finishes the process (see Section 13a/15) | Partner A |
| **Business Maintenance & Compliance** *(recurring, unlocked once Business Registration is complete)* | CAC Annual Returns filing, Tax Filing (VAT/CIT/PIT returns), Tax Clearance Certificate (TCC), state-specific permits (LASAA, local govt); the in-house **Tax Suite** (PAYE Calculator, VAT Estimator, Mini Payroll, Payslip Generator, Annual Tax Certificate, NRS/FIRS E-Invoicing — see Section 12); seller-defined **Trackers** plus the "Licences & Documents" category; and the **Compliance Calendar** tying all of the above into one sorted view (see Section 13) | Partner A (Annual Returns), Partner B (Tax Filing/TCC/permits), SellersPoint in-house (Tax Suite, Trackers, Calendar) |
| **Verification** *(standalone — not an onboarding step)* | TIN, BVN, NIN verification; CAC Business Name availability check; CAC business status lookup; document authenticity check. For a third party's details, or a business that registered somewhere other than SellersPoint Docs — **not something a business that registered through us needs to do for its own TIN, since that's already known from Business Registration** | Automated (vendor API), with Partner A as an internal fallback only for document authenticity on high-stakes documents |

**Self-Serve Documents & Tools** *(Templates, demoted from primary nav in v9)* sits outside the three pillars — Contract/Agreement Templates and the Proposal/Quote Generator, fulfilled entirely in-house, free, and not gated by registration status. See the Templates positioning note below.

**Trademark registration — removed in v11.** It's no longer part of the catalog, the data model, or the dashboard.
...
**Nav grouping, current as of v16:** Overview, then **Verification** — standalone, ungrouped, always usable, right where a seller would look for it early — then **Business Registration**, itself ungrouped since it's a single item. Below that, a **"Maintenance & Compliance"** group holds Compliance Calendar, Tax Tools, Trackers, and Document Vault — the four views that map directly onto the Business Maintenance & Compliance pillar above, and the four that are locked until onboarding is complete (see the v16 gate note just below). This replaced the v14 "Onboarding" (Business Registration + Verification) / "Manage" (Tax Tools, Trackers, Document Vault) split — Verification moved out of the onboarding group because it never was an onboarding step; grouping it there in v14 was a mistake this pass corrects.

**v15/v16: Business Registration is a one-time process that ends at SCUML, and nothing in Maintenance & Compliance makes sense before it's done — so it's an actual gate, not just a label.** `isOnboardingComplete()` is true only once `regStatus === "approved"` **and** `scuml.status === "approved"` — SCUML is explicitly the last step, matching how it works in reality (issued once by the EFCC, after the underlying Business Name/LLC/Partnership registration is already done). Until both are true:
- Compliance Calendar, Tax Tools, Trackers, and Document Vault show a 🔒 next to their nav label and are visibly dimmed. (Calendar joined the locked set in v16 — see below — Tax Tools/Trackers/Vault were already locked as of v15.)
- Clicking any of them — from the sidebar, an Overview quick action, or an alert's action button, since all of these route through one shared `goto()` — redirects to Business Registration instead, with a toast explaining why. Nothing is a dead end; every locked entry point has a working escape hatch back to the thing that unlocks it.
- The Overview "Onboarding" card and the SCUML congrats banner are *computed* from the same three fields (`regStatus`, `tinIssued`, `scuml.status`) that drive the gate, not separately hardcoded — so the progress bar, step checklist, congrats message, and nav locks can never drift out of sync with each other or with what's actually navigable.
- **Verification stays unlocked throughout, deliberately** — it's a standalone service now (see the pillar table above), not an onboarding step, so completion status has no bearing on whether it's reachable. Templates also stay unlocked (reachable via the Overview quick action) since generic legal templates don't depend on a TIN or SCUML certificate existing.
- **Compliance Calendar moved from unlocked to locked in v16.** It was kept open in v15 on the reasoning that previewing statutory deadlines pre-registration was motivating; v16 reverses that, since the Calendar is now explicitly filed under the Maintenance & Compliance pillar and its statutory items (VAT/PAYE/WHT/CIT/PIT) presuppose a TIN, which a not-yet-registered business doesn't have.

**Business's own TIN is known automatically once Business Registration completes — v16.** The business record carries `tin` as a real value from the moment `tinIssued` is true, not `null` until a manual Verification check populates it. A seller can still open Verification and run a TIN check on their own TIN if they want independent confirmation (it's a normal, unrestricted action), but the platform doesn't require it and doesn't gate anything on it — TIN auto-fill (Section 13b) now works immediately after registration, not only after a seller happens to visit Verification first.
...
|---|---|---|
| **Partner A — Corporate & Legal** | Business Name, LLC, or Partnership registration — **TIN generated and included as part of this package, not fulfilled or billed as a separate line item (v16)** — SCUML registration, plus CAC Annual Returns filing | Law firm or CAC-accredited agent |
| **Partner B — Tax & Regulatory** | Tax Filing and Tax Clearance Certificate, plus state-specific permits. **No longer owns TIN generation as of v16** — that moved to Partner A, bundled into the Business Registration package it's actually part of | Chartered accountant (ICAN) or licensed tax/regulatory-filing agent |
...
**Verification is standalone and almost entirely automated, not partner-routed (reframed in v16).** It used to sit alongside Business Registration as a second onboarding-adjacent step ("verify your TIN"); it's now explicitly decoupled — see Section 2's pillar table. Verification API vendors exist that cover TIN, BVN, NIN, and CAC lookups under one integration — QoreID's public docs confirm all four — so this whole tier can run on SellersPoint's own infrastructure with near-instant turnaround, instead of the ~12 hours a manual agent needs. Only document authenticity verification on a high-stakes document might still warrant a manual cross-check as a fallback. Its real use cases are things Business Registration itself doesn't cover: a third party's TIN/BVN/NIN, or confirming details for a business that registered somewhere other than SellersPoint Docs.
...
- **Pre-registration sellers** (primary/volume) — start at Business Registration (TIN included automatically).
...
> Design the product and automation architecture for **SellersPoint Docs**, organized around three pillars: (1) **Business Registration** (one-time) — CAC Business Name, LLC incorporation, or Partnership registration, with TIN generated and included automatically as part of the package (never billed or verified as a separate step), plus SCUML registration as the step that finishes the process — fulfilled behind the scenes by a licensed corporate partner; (2) **Business Maintenance & Compliance** (recurring, unlocked once registration is complete) — CAC Annual Returns filing, periodic Tax Filing, Tax Clearance Certificates, and state-specific permits fulfilled behind the scenes by licensed corporate/tax partners, plus an in-house **Tax Suite** (a free PAYE Calculator and VAT Estimator, and a paid tier: Mini Payroll, Payslip Generator, Annual Tax Certificate, NRS/FIRS E-Invoicing, built on Nigeria's current tax law), seller-managed **Trackers** (including a **Licences & Documents** category for items that can be actioned into real fulfillment), and a **Compliance Calendar** unifying every deadline from Trackers, the Tax Suite, and Business Registration into one sorted view; (3) **Verification**, standalone — TIN/BVN/NIN/CAC checks fully automated via a KYC vendor API with near-instant turnaround, for a third party's details or a business that registered elsewhere, explicitly *not* a required onboarding step for a business that registered through us. Outside the three pillars: self-serve Templates that show a **basic, ready-to-use document immediately**, with AI-assisted improvement as an optional second step, constrained to a pre-vetted clause library — AI must never draft novel legal clauses. **No partner name may ever appear in seller-facing copy** — all fulfillment partners are invisible backend infrastructure; the seller only ever deals with SellersPoint. Payment for backend-fulfilled work is held in escrow-style and released on delivery milestones, not upfront. Every document produced or uploaded lands in one unified, NDPR-compliant Document Vault.
...
**CAC's own VAS registration API — confirmed real and end-to-end, not a lookup/verification tool (found and inspected directly, not just referenced from docs):**
- **Base URLs:** sandbox `https://vasapp.oasisproducts.ng`, production `https://vasapp.cac.gov.ng`. Auth is a single `X_API_KEY` header — no OAuth.
- **Full coverage, not partial:** name reservation → AI-assisted memorandum-object generation/analysis (CAC's *own* AI drafts and validates the "objects of memorandum" clauses — directly overlaps with the AI Boundary design in Section 4, worth revisiting once this is integrated) → company registration (type/nature of business/address/objects, with a `priorityService` param suggesting a paid expedite tier) → share registration → affiliate registration (directors/secretary/witness/presenter/shareholder, individual or corporate, with ID/signature/photo/PEP screening) → PSC (beneficial ownership) registration → submit → stamp duty payment (generates and verifies a Remita RRR, i.e. the government fee itself is collected through the API) → status checks and update/delete on every entity.
- **Not covered by this API:** SCUML (EFCC-issued, a separate system entirely) and, per the brief's existing assumption, TIN issuance may still be a distinct FIRS-side step — both need their own confirmation, same open question as before.
- **The one real unknown: access.** CAC runs an Accredited Agent program (`icrp.cac.gov.ng`) gating who's legally allowed to file registrations at all — API access is very likely gated behind that same accreditation, not open to any developer with an email address. This is a partnerships/accreditation conversation with CAC, not an engineering unknown anymore.

**Phasing decision (updated):** direct CAC VAS API integration is no longer a "maybe, someday" fast-follow — the API itself is confirmed to exist, work, and cover the full registration flow end-to-end. The only remaining blocker between "interesting API" and "actually usable" is securing CAC accreditation/API-key access. Until that's secured, Partner A still handles registration manually through CAC's own portal exactly as originally planned; the moment accreditation clears, the corporate-partner role in Workflow A (Section 3, stages 4–7) can shift from "files everything by hand" to "reviews and signs off on API calls SellersPoint's own systems make" — same legal accountability, far less manual work, much faster turnaround.
...
## 13. Compliance Calendar (upgraded in v9, layout redesigned in v12 and v14)
...
**Page layout (v14): two columns, not one stacked column.** The view splits into a left column (Next deadline hero + the two stacked month cards + legend) and a right column (the Upcoming card), side by side. v12 had briefly made everything — hero, months, and Upcoming — one long stacked column; v14 pulls Upcoming out into its own column alongside the calendar instead, since the two serve different reading modes (browse the calendar visually vs. scan a prioritized list) and don't need to compete for the same vertical space. On narrow screens the two columns collapse back to one, calendar first.

**Three visible pieces (within that layout):**
1. **Next deadline hero stat** — the single soonest statutory obligation, shown prominently ("12 days · 21 Jul") with a direct action, at the top of the left column.
2. **Two-month grid view, always stacked (v12), inside the left column** — this month, then next month directly below it, separated by a visible divider and each carrying its own full month-name header (e.g. "July 2026" / "August 2026"). Originally this sat in a two-column *month* layout that only stacked at narrow widths as a responsive side-effect; v12 made the stacked order permanent and deliberate so the relationship between the two months always reads clearly, regardless of screen width — that stacking is unrelated to, and unaffected by, the v14 page-level column split above. This exists specifically because a single-month view hides real near-term deadlines that fall just past the month boundary — e.g. on 18 July, the next PAYE remittance (10th of the month) has already passed for July and doesn't land until 10 August, invisible on a one-month grid but clearly visible on two.
...
4. **Merged upcoming list, in the right column, fixed two-column row layout (v12)** — statutory deadlines plus open Trackers, sorted soonest-first, each tagged by source (🏛️ statutory / 📌 tracker) with a contextual action ("File now" for filing-type obligations, "Estimate now" for VAT, "View tracker" for self-tracked items). Each row is its own two-column grid — description on the left (wraps freely), due-date badge and action on the right (fixed width, pinned to the row's right edge) — so a long obligation name or note can never push the date out of alignment or wrap it onto its own line, which the original flex layout allowed. (This per-row grid is a different thing from the v14 page-level column split — the row layout was unchanged by it.)
...
- **Package & Pricing management** — every priced service as an editable table: seller-facing price, internal partner/vendor cost, and computed margin. **Grouped into the same three pillars as the catalog (v16)** — Business Registration (Business Name/LLC/Partnership Registration, each marked "TIN included," plus SCUML Registration and the New Business Bundle), Business Maintenance & Compliance (CAC Annual Returns Filing, Tax Filing, Tax Clearance Certificate, the Tax Suite paid tier), and Verification, standalone (TIN/BVN/NIN/CAC Verification, Document Authenticity Check) — with a subheader row breaking up each group in the table so the pillar structure is visible to staff, not just implied by row order. This is where "See bundle pricing" and "Bundle pricing would open here" on the customer dashboard actually get their numbers from — pricing is set once, here, never exposed as an editable surface to sellers. Figures shown in the mockup are current working examples for planning, not published rates.
...
**v14 note:** the Compliance Calendar view is now two columns side by side — calendar (hero stat + the two stacked month cards) on the left, the Upcoming list on its own in a matching card on the right — instead of one long stacked column (see Section 13). The sidebar nav also gained two group labels with no route changes: **Onboarding** above Business Registration and Verification, **Manage** above Tax Tools, Trackers, and Document Vault, with Overview and Compliance Calendar left ungrouped above both.

**v15 note:** those nav groups are now a real gate, not just labels — Tax Tools, Trackers, and Document Vault stay locked (🔒, dimmed, redirect-on-click) until Business Registration reaches SCUML-approved, since that's the one-time process everything else depends on. The Overview "Onboarding" card and SCUML congrats banner are computed from the same `regStatus`/`tinIssued`/`scuml.status` fields the gate itself checks, replacing three previously-independent hardcoded fields on the business record. See Section 2.

**v16 note:** the catalog is now three pillars — Business Registration, Business Maintenance & Compliance, Verification (standalone) — and the dashboard nav follows suit: Verification moved out of the old "Onboarding" group to sit ungrouped, right after Overview, since it was never actually an onboarding step; the locked group is relabeled **"Maintenance & Compliance"** and now includes Compliance Calendar alongside Tax Tools, Trackers, and Document Vault. The business record's `tin` field is populated the moment registration completes (`tinIssued`) instead of staying `null` until a manual Verification check — TIN comes bundled with the registration package, so there's nothing left to verify on the way in. `sellerspoint-docs-admin.html`'s Package & Pricing table is regrouped into the same three pillars with subheader rows. See Section 2 and Section 3.
