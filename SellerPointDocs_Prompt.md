# SellersPoint Docs — Product & Automation Brief (v14)
...
**Onboarding vs. Manage — sidebar grouping added in v14.** Business Registration and Verification are the one-time-per-business setup work — get registered, get verified — so the sidebar now labels them **Onboarding**. Tax Tools, Trackers, and Document Vault are what a seller comes back to repeatedly after that's done, labeled **Manage**. Overview and the Compliance Calendar sit above both groups, ungrouped, since they're dashboard-level surfaces a seller might open regardless of which phase they're in. This is a navigation label change only — no view, route, or data changed — but it makes the product's own shape ("register and verify once, then manage on an ongoing basis") visible in the UI instead of implicit.
