...
let owner = { name: "SellPoint", provider: "Manual bank transfer", link: "", details: "" };
let businesses = [];
let payments = [];
let authToken = null;
...
async function loadAll() {
  const [ownerRes, businessesRes, paymentsRes] = await Promise.all([
    api("GET", "/api/owner"),
...
$("ownerPaymentForm").onsubmit = async (e) => {
  ...
  toast("SellPoint payment details saved");
  render();
};
