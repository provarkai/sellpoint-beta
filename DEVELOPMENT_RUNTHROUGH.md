# SellersPoint — Development Run-Through

_Last updated: 2026-07-07_

A single document to get oriented fast: what SellersPoint is, what's actually
...
## 1. What SellersPoint is

SellersPoint is a multi-tenant SaaS app for small Nigerian businesses to run
...
## 2. Project history (milestones, not a full commit log — see `git log`)

1. **Initial import**: a frontend-only prototype. Static HTML/JS,
   `localStorage` for persistence, one shared business, no real auth.
2. **Multi-tenant SaaS rewrite**: added a real Node/Express + Postgres
   (Supabase) backend, Supabase Auth (email+password) replacing shared HTTP
   Basic Auth, `business_id` scoping on every table, and real Paystack
   checkout (initialize / webhook / verify-by-reference). No longer a
   static site from this point on.
3. **SellersPoint rebrand + pricing simplification**: renamed from
   SellPoint; pricing collapsed to Starter + Growth by default (see §7);
   Paystack-only checkout (manual bank-transfer flow removed entirely).
4. **Input validation + automated tests + CI**: `server/validate.js`,
   `server/__tests__/`, `.github/workflows/ci.yml`.
...
| Payments | Paystack only (checkout + webhook + verify-by-reference) |
| Testing | Node's built-in `node:test` (`server/__tests__/`), CI via GitHub Actions on every push/PR |
| Hosting target | Railway or Render (needs a persistent Node process — static hosts won't work) |

No build tooling - this is intentionally minimal (no framework, no
bundler). See gaps below for what's still missing.
...
backend.html / backend.js   SellersPoint's OWN cross-tenant platform-admin panel (not per-business)
...
server/pricing.js           plan tiers (starter/growth/pro/business/enterprise) + order-limit + visibleTiers()
server/validate.js          requireString/requireNumber - dependency-free input validation
...
server/__tests__/           node:test suite (validation, pricing, webhook signatures)

CLAUDE.md                   strategic roadmap + business-stage pricing plan
...
- `owner_payment` — a genuine singleton row: SellersPoint's *own* payout
...
Without `PAYSTACK_SECRET_KEY`/`PAYSTACK_PUBLIC_KEY` set,
`/api/payments/initialize` returns a clean 400 - there is no manual
payment fallback anymore, checkout is Paystack-only.

Plan tiers (`server/pricing.js`): Starter (free, 30 orders/mo), Growth
(₦5,000/mo), Pro (₦8,500/mo), Business (₦20,000/mo), Enterprise (custom,
no self-serve checkout). Yearly = 10x monthly (2 months free). **Only
Starter + Growth are offered by default** - Pro/Business/Enterprise stay
hidden until `platform_settings.extended_pricing_enabled` is turned on
from `backend.html`; see `CLAUDE.md`'s pricing strategy section for why.
...
- Platform-admin panel (`backend.html`) listing all businesses/payments and
  editing SellersPoint's own payout details.
...
3. **File uploads (logo, digital-product downloads) are base64 in
   ...
   over time.
4. **Single payment provider** — Paystack only, and it's the only checkout
   path now that the manual bank-transfer fallback has been removed;
   Flutterwave listed as a good next add for redundancy/coverage.
5. **Pro/Business tiers don't gate anything Growth doesn't have** — see
   `CLAUDE.md`'s pricing strategy section; this is why they're hidden by
   default rather than just discounted or removed.
6. **No downloadable receipt PDF**, no customer-facing checkout page per
   order.
7. **No expiry-based downgrade notifications** before `plan_expires_at` lapses.
8. **NGN-only, no i18n/multi-currency.**
9. **Not deployed anywhere yet** — `HOSTING.md` documents Railway/Render as
...
10. **Test coverage is unit-level only** — `server/__tests__/` covers pure
...
Without `PAYSTACK_SECRET_KEY`/`PAYSTACK_PUBLIC_KEY` set, checkout requests
return a 400 - there's no fallback, so set those two vars to actually test
the upgrade flow.
...
16. Tier feature gating (staff seat limits, AI usage metering, a reports
    view) before turning on the Pro/Business/Enterprise tiers for real
    customers - see `CLAUDE.md`'s pricing strategy section.

## 12. Where to look for more detail

- **Strategic roadmap, slice priority order, GTM, pricing strategy** →
  `CLAUDE.md`
